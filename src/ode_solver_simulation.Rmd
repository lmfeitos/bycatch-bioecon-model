...
df_v <- data.frame(N = rep(N, 3), 
                 V = c(N^(beta_1-1), N^(beta_2-1), N^(beta_3 - 1)),
                 beta = rep(c("Not hyperstable", "Hyperstable", "Hyperdepleted"), each = length(N)))

df_cpue <- data.frame(N = rep(N, 3), 
                 CPUE = c(q * N^beta_1, q * N^beta_2, q * N^beta_3),
                 beta = rep(c("Not hyperstable", "Hyperstable", "Hyperdepleted"), each = length(N)))
