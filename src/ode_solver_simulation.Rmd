...
out1 <- out1 %>% 
  ...
  mutate(catch_s1 = q_s * N_s1^beta_s1 * E1,
         ...
         cpue_w1 = catch_w1 / E1,
         cost1 = cost * E1,
         pi_st1 = catch_s1 * A_s * (q_s * N_s1^beta_s1 * E1)^-f_s1 - (catch_s1 * cost),
         pi_w1 = catch_w1 * A_w * (q_w * N_w1^beta_w1 * E1)^-f_w1 - (catch_w1 * cost),
         pi_sum1 = rowSums(across(c(pi_st1, pi_w1)))) %>% 
...
out2 <- out2 %>% 
  ...
  mutate(catch_s2 = q_s * N_s2^beta_s2 * E2,
         ...
         cpue_w2 = catch_w2 / E2,
         cost2 = cost * E2,
         pi_st2 = catch_s2 * A_s * (q_s * N_s2^beta_s2 * E2)^-f_s2 - (catch_s2 * cost),
         pi_w2 = catch_w2 * A_w * (q_w * N_w2^beta_w2 * E2)^-f_w2 - (catch_w2 * cost),
         pi_sum2 = rowSums(across(c(pi_st2, pi_w2)))) %>% 
  pivot_longer(cols = dplyr::starts_with("N_"),
               names_to = "stock",
               values_to = "stock_size") %>% 
  pivot_longer(cols = dplyr::starts_with("E"),
...
out3 <- out3 %>% as.data.frame() %>% 
  ...
  mutate(catch_s3 = q_s * N_s3^beta_s3 * E3,
         ...
         cpue_w3 = catch_w3 / E3,
         cost3 = cost * E3,
         pi_st3 = catch_s3 * A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 - (catch_s3 * cost),
         pi_w3 = catch_w3 * A_w * (q_w * N_w3^beta_w3 * E3)^-f_w3 - (catch_w3 * cost)) %>% 
  mutate_at(vars(pi_w3), ~replace(., is.nan(.), 0)) %>% 
  mutate(pi_sum3 = rowSums(across(c(pi_st3, pi_w3)))) %>% 
  pivot_longer(cols = dplyr::starts_with("N_"),
               names_to = "stock",
               values_to = "stock_size") %>% 
  pivot_longer(cols = dplyr::starts_with("E"),
...
out4 <- out4 %>% 
  ...
  mutate(catch_s4 = q_s * N_s4^beta_s4 * E4,
         ...
         cpue_w4 = catch_w4 / E4,
         cost4 = cost * E4,
         pi_st4 = catch_s4 * A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 - (catch_s4 * cost),
         pi_w4 = catch_w4 * A_w * (q_w * N_w4^beta_w4 * E4)^-f_w4 - (catch_w4 * cost)) %>% 
  mutate_at(vars(pi_w4), ~replace(., is.nan(.), 0)) %>% 
  mutate(pi_sum4 = rowSums(across(c(pi_st4, pi_w4)))) %>% 
  pivot_longer(cols = dplyr::starts_with("N_"),
               names_to = "stock",
               values_to = "stock_size") %>% 
  pivot_longer(cols = dplyr::starts_with("E"),
...
scenario_5 <- function(time, state, parameters) {
  with(as.list(c(state, parameters)), {
    ...
    dE5 <- alpha * E5 * (A_s * (q_s * N_s5^beta_s5 * E5)^-f_s5 * q_s * N_s5^beta_s5 +
      ...
      dN_s5 <- r_s * N_s5 * (1 - (N_s5/k_s)) - (q_s * N_s5^beta_s5 * E5)
      dN_w5 <- 0
      dE5 <- alpha * E5 * (A_s * (q_s * N_s5^beta_s5 * E5)^-f_s5 * q_s * N_s5^beta_s5 - (c))
...
# Define the initial state and parameters
state5 <- c(N_s5 = 4, N_w5 = 2, E5 = 0.5) #, C_s1 = 0, C_w1 = 0, Pi_s1 = 0, Pi_w1 = 0
parameters_5 <- c(q_s = 0.02, q_w = 0.02, k_s = 4, k_w = 2, r_s = 1, r_w = 0.99, alpha = 1,
                c = 0.05, A_w = 1, A_s = 1, beta_w5 = 0.56, beta_s5 = 0.2, f_w5 = 0.1, f_s5 = 0.815)

# Solve ODEs
out5 <- ode(y = state5, times = times, func = scenario_5, parms = parameters_5)

out5 <- out5 %>% as.data.frame() %>% 
  mutate(N_w5 = case_when(
    N_w5 < 0 ~ 0,
    TRUE ~ N_w5
  )) %>% 
  mutate(catch_s5 = q_s * N_s5^beta_s5 * E5,
  ...
  mutate_at(vars(pi_w5), ~replace(., is.nan(.), 0)) %>% 
  mutate(pi_sum5 = rowSums(across(c(pi_st5, pi_w5)))) %>% 
  pivot_longer(cols = dplyr::starts_with("N_"),
               names_to = "stock",
               values_to = "stock_size") %>% 
  pivot_longer(cols = dplyr::starts_with("E"),
...
df_v <- data.frame(N = rep(N, 2), 
                 ...
                 beta = rep(c("No hyperstability", "Hyperstability"), each = length(N)))
