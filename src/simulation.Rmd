...
```{r}
ps_prices <- output_price %>% 
  filter(stock_prices == "p_s")

pw_prices <- output_price %>% 
  filter(stock_prices == "p_w")

ggplot() +
  # geom_point(data = output_price, 
  #          aes(x = prices, y = values, color = stock_prices),
  #          alpha = .4) +
  # geom_line(data = output_price, 
  #          aes(x = prices, y = values, color = stock_prices),
  #          alpha = .4) +
  geom_smooth(data = output_price, 
           aes(x = prices, y = values, color = variables)) +
...
output_q <- output_q_tidy %>% 
  select(scenario_id, variables, values, stock_q, q, q_ratio) %>% 
  distinct(variables, values, stock_q, q, q_ratio)
```
...
output_q %>% 
  mutate(variables = case_when(
    variables == "e" ~ "effort",
    variables == "N_s" ~ "Strong stock",
    variables == "N_w" ~ "Weak stock"
  )) %>% 
  ggplot() +
  geom_point(aes(x = q, y = values, color = stock_q),
             alpha = .2) +
  geom_line(aes(x = q, y = values, color = stock_q),
             alpha = .2) +
  facet_wrap(~ variables,
             scales = "free_y") +
  labs(x = "Catchability ranges",
       y = "Variable size (mil)",
...
output_growth %>% 
  ggplot() +
  geom_point(aes(x = r, y = values, color = stock_growth_r),
           alpha = 0.4) +
  facet_wrap(~ variables,
             scales = "free_y") +
  labs(x = "r ranges",
