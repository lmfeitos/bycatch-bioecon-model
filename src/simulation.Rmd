...
run_model <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c){
 ...
 # Build effort dataframe
  effort_df <- data.frame(r_s = r_s, k_s = k_s, q_s = q_s, p_s = p_s,
                          r_w = r_w, k_w = k_w, q_w = q_w, p_w = p_w, c = c)
 
  # Build weak stock data frame
  n_weak_df <- data.frame(r_s = r_s, k_s = k_s, q_s = q_s, p_s = p_s,
                          r_w = r_w, k_w = k_w, q_w = q_w, p_w = p_w, c = c)
   
  # Build strong stock data frame
  n_strong_df <- data.frame(r_s = r_s, k_s = k_s, q_s = q_s, p_s = p_s,
                          r_w = r_w, k_w = k_w, q_w = q_w, p_w = p_w, c = c)
  
  # Build output
  output <- list(effort_df, n_weak_df, n_strong_df)
...
for(i in 1:nrow(scenarios_df)){
  ...
  # Run model
  output <- run_model(r_s = r_s, k_s = k_s, q_s = q_s, p_s = p_s,
                          r_w = r_w, k_w = k_w, q_w = q_w, p_w = p_w, c = c)
