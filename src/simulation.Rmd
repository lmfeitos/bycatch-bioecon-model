...
scenarios_df <- expand.grid(r_s = seq(0.1, 1, 0.2),
                            r_w = seq(0.1, 1, 0.2),
                            k_s = seq(1, 5, 1),
                            k_w = seq(1, 5, 1),
                            q_s = seq(0.1, 1, 0.2),
...
output_q %>% 
  ggplot() +
  geom_col(aes(x = q, y = values, fill = variables),
           alpha = 0.4) +
  labs(x = "Q ranges",
       y = "Variable size (mil)",
       title = "Simulation of q ranges on variable size") +
  theme_bw() 
```
...
output_growth %>% 
  ggplot() +
  geom_col(aes(x = r, y = values, fill = variables),
           alpha = 0.4) +
  facet_wrap(~ variables,
             scales = "free_y") +
  labs(x = "r ranges",
