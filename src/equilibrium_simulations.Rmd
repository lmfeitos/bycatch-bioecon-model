...
run_model_test <- function(r_s, k_s, q_s1, q_s2, p_s, r_w, k_w, q_w1, q_w2, p_w, c){
  # Weak stock equilibrium points when prices are constants
  N_w_multi <- (c * q_s1 - c * q_w2) / 
    (p_w * q_s1 * q_w1 - p_w * q_s2 * q_w2)
  
  # Strong stock equilibrium points when prices are constants 
  N_s_multi <- (c * (- q_s2 + q_w1)) /
    (p_s * (q_s1 * q_w1 - q_s2 * q_w2))
  
  # First effort equilibrium point when prices are constants
  E_multi_1 <- (c * k_w * p_w * (q_s2 - q_w1) * q_w2 * r_s + c * k_s * p_s * q_w1 * (q_s1 - q_w2) * r_w - k_s * k_w * p_s * p_w * (q_s1 * q_w1 - q_s2 * q_w2) * (-q_w2 * r_s + q_w1 * r_w)) / (k_s * k_w * p_s * p_w * (q_s2 * q_w1 - q_s1 * q_w2) * (-q_s1 * q_w1 + q_s2 * q_w2))
    
  # Second effort equilibrium point when prices are constants
  E_multi_2 <- ((c * k_w * p_w * q_s2 * (-q_s2 + q_w1) * r_s + c * k_s * p_s * q_s1 * (-q_s1 + q_w2) * r_w + k_s * k_w * p_s * p_w * (q_s1 * q_w1 - q_s2 * q_w2) * (-q_s2 * r_s + q_s1 * r_w))) / ((k_s * k_w * p_s * p_w * (q_s2 * q_w1 - q_s1 * q_w2) * (-q_s1 * q_w1 + q_s2 * q_w2)))
...
output_1 <- scenarios_df %>%
  ...
  mutate(output = list(run_model(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                                 q_s = q_s, q_w1 = q_w1, p_s = p_s, p_w = p_w, 
                                 c = c))) %>% 
  unnest(output) %>% 
