...
run_model <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c){
  ...
  #Es <- (r_s / q_s * (1 - c / p_s * q_s * k_s))
  
  # Single species effort equilibrium for the weak stock  
  #Ew <- 1 / (q_w / r_w)
 ...
 #  #C_s2 <- q_s * N_s2 * Es
 #    
 #  # Weak stock price as a function of catch
 #  p_w_star <- C_w
 # 
 #  # Strong stockpPrice as a function of catch
 #  p_s_star <- C_s
 # 
 #  # Weak stock price as a function of catch
 #  #p_w_star2 <- C_w2
 ...
 #  #p_s_star2 <- C_s2
 #  
 #  # Derive weak stock biomass when weak stock price is a function of N_w catch
 # 
 #  N_w_star_1 <- (k_w * (c * q_w * r_s + k_s * p_s * q_s * (- q_w * r_s + q_s * r_w))) / 
 #    (k_w * p_w_star * q_w^2 * r_s + k_s * p_s * q_s^2 * r_w)
 #  
 #  # Derive effort when weak stock price is a function of N_w catch
 #  E_star_1 <- ((- c + k_s * p_s * q_s + k_w * p_w_star * q_w) * r_s * r_w)  /
 #   (k_w * p_w_star * q_w^2 * r_s + k_s * p_s * q_s^2 * r_w) 
 #  
 # # Derive strong stock biomass when weak stock price is a function of N_w catch
 # 
 #  N_s_star_1 <- (k_s * (c * q_s * r_w + k_w * p_w_star * q_w * (q_w * r_s - q_s * r_w))) /
 #     (k_w * p_w_star * q_w^2 * r_s + k_s * p_s * q_s^2 * r_w)
 #  
 #  # Derive equilibrium abundance of weak stock with p_s as a function of N_s catch
 #  N_w_star_2 <- (k_w * (c * q_w * r_s + k_s * p_s_star * q_s * (- q_w * r_s + q_s * r_w))) / 
 #    (k_w * p_w * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w)
 #  
 #  # Derive effort with p_s as a function of N_s catch
 #  E_star_2 <- ((- c + k_s * p_s_star * q_s + k_w * p_w * q_w) * r_s * r_w)  /
 #   (k_w * p_w * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w) 
 #  
 #  # Derive equilibrium abundance of strong stock with p_s as a function of N_s catch
 # 
 #  N_s_star_2 <- (k_s * (c * q_s * r_w + k_w * p_w * q_w * (q_w * r_s - q_s * r_w))) /
 #     (k_w * p_w * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w)
 #  
 #  # Derive equilibrium abundance of weak stock with p_s and p_w as functions of N_s and N_w catch
 #  N_w_star_3 <- (k_w * (c * q_w * r_s + k_s * p_s_star * q_s * (- q_w * r_s + q_s * r_w))) / 
 #    (k_w * p_w_star * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w)
 #  
 #  # Derive effort with p_s and p_w as functions of N_s and N_w  catch
 #  E_star_3 <- ((- c + k_s * p_s_star * q_s + k_w * p_w_star * q_w) * r_s * r_w)  /
 #   (k_w * p_w_star * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w) 
 #  
 #  # Derive equilibrium abundance of strong stock with p_s and p_w as functions of N_s and N_w  catch
 # 
 #  N_s_star_3 <- (k_s * (c * q_s * r_w + k_w * p_w_star * q_w * (q_w * r_s - q_s * r_w))) /
 #     (k_w * p_w_star * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w)
 #  
  
  # Build output
  output <- data.frame(E = E, N_s = N_s, N_w = N_w)
                       # p_w_star = p_w_star, p_s_star = p_s_star, C_w = C_w, C_s = C_s,
                       # N_w_star_1 = N_w_star_1, E_star_1 = E_star_1, N_s_star_1 = N_s_star_1,
                       # N_w_star_2 = N_w_star_2, E_star_2 = E_star_2, N_s_star_2 = N_s_star_2,
                       # N_w_star_3 = N_w_star_3, E_star_3 = E_star_3, N_s_star_3 = N_s_star_3)
...
run_model_2 <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c){
  
  # Vulnerability of the strong stock
  Vs <- q_s / r_s
  
  # Vulnerability of the weak stock
  Vw <- q_w / r_w
  ...
  # Single species equilibrium for the weak stock 
  N_w_fcn <- (c / (p_w_star2 * q_w))
  
  # Single species equilibrium for the strong stock  
  N_s_fcn <- (c / (p_s_star2 * q_s))
  
  E_s_fcn <- ((1 - c / (p_s_star2 * q_s * k_s)) / Vs)

  output <- data.frame(Vs = Vs, Vw = Vw, Es = Es, Ew = Ew, N_s2 = N_s2, N_w2 = N_w2,
                       p_w_star2 = p_w_star2, p_s_star2 = p_s_star2, C_w2 = C_w2, C_s2 = C_s2,
                       N_s_fcn = N_s_fcn, N_w_fcn = N_w_fcn, E_s_fcn = E_s_fcn)
...
# Building df of price range scenarios
scenarios_df <- expand.grid(p_s = seq(0.3, 10, 0.2),
                            p_w = seq(1, 15, 2),
                            r_s = 1,
                            r_w = seq(0.75, 0.999, 0.01),
                            q_s = 0.05,
                            q_w = 0.05,
                            k_s = 4,
                            k_w = 2,
...
#write_csv(output_join_test, file = file.path(outdir, "output_s_ranges_w_ranges.csv"), col_names = T)
```
