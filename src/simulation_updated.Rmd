...
run_model_msy <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c){
  ...
  # Single species equilibrium for the strong stock  
  N_s_msy <- (k_s * (c * q_s * r_w + k_w * p_w * q_w * (q_w * r_s - q_s * r_w))) /
     (2 * k_w * p_w * q_w^2 * r_s + k_s * p_s * q_s^2 * r_w)

  # Single species effort equilibrium for the strong stock  
  E_s_single_msy <- r_s / (2 * q_s * (1 - c / (2 * p_s * q_s * k_s)))
  ...
  if(E_multi < r_s/(2*q_s)){
    
    E = E_multi
  
  } else if(E_s_single_msy < r_s/(2*q_s)){
...
# pmap version
params_list <- scenarios_df %>% 
  select(p_s, p_w, r_s, r_w, q_s, q_w, k_s, k_w, c) %>% 
  transpose() %>% 
  as.list()

output_list <- pmap(params_list, function(p_s, p_w, r_s, r_w, q_s, q_w, k_s, k_w, c){
  run_model(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
            q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w, 
            c = c)
})

output_1_pmap <- bind_rows(output_list, .id = "scenario_id")

# calculating speed of each alternative

#vectorized

microbenchmark(
  output_vec <- run_model(scenarios_df$r_s, scenarios_df$k_s, scenarios_df$q_s, 
                                  scenarios_df$p_s, scenarios_df$r_w, scenarios_df$k_w, 
                                  scenarios_df$q_w, scenarios_df$p_w, scenarios_df$c),
  times = 10)
)

# Using pmap
microbenchmark(
  output_pmap <- pmap(scenarios_df, run_model_msy),
  times = 10
)
```
...
# Building df of price range scenarios
scenarios_df_msy1 <- expand.grid(p_s = seq(1, 10, 0.2),
                                 p_w = c(0, 1),
                                 r_s = seq(0.1, 1, 0.01),
                                 r_w = seq(0.1, 0.9, 0.01),
                                 q_s = 0.02,
...
scenarios_df_subs1 <- expand.grid(p_s = seq(1, 10, 0.2),
                                  p_w = 1,
                                  r_s = seq(0.1, 1, 0.01),
                                  r_w = seq(0.1, 0.9, 0.01),
                                  q_s = 0.02,
...
output_join1 <- output_sub1 %>% 
  rename(scenario_id = sc_id) %>% 
  left_join(scenarios_df_subs1, by = "scenario_id") %>% 
  relocate(scenario_id, .before = E)

output_join0 <- output_sub0 %>% 
  rename(scenario_id = sc_id) %>% 
  left_join(scenarios_df_subs1, by = "scenario_id") %>% 
  relocate(scenario_id, .before = E)

output_sub1_full <- bind_rows(output_join0, output_join1)

write_csv(output_sub1_full, file = file.path(outdir, "output_sub1.csv"), col_names = T)
...
scenarios_df_subs2 <- expand.grid(p_s = 2,
                                  p_w = seq(1, 10, 0.2),
                                  r_s = seq(0.1, 1, 0.01),
                                  r_w = seq(0.1, 0.9, 0.01),
                                  q_s = 0.02,
...
output_join1.1 <- output_sub2 %>% 
  rename(scenario_id = sc_id) %>% 
  left_join(scenarios_df_subs2, by = "scenario_id") %>% 
  relocate(scenario_id, .before = E)

output_join_full <- bind_rows(output_join1, output_join1.1)

#write_csv(output_join_full, file = file.path(outdir, "output_sub2.csv"), col_names = T)
```
