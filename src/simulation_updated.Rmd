...
output_1_msy <- scenarios_df_msy1 %>% 
  rowwise() %>% 
  mutate(output = list(run_model_msy(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                                     q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w, 
                                     c = c))) %>% 
  unnest(output) %>% 
  as.data.frame()
...
output_2_msy <- scenarios_df_msy2 %>% 
  filter(r_s > r_w) %>% 
  rowwise() %>% 
  mutate(output = list(run_model_msy(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                                     q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w, 
