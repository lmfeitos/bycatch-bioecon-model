...
run_model <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c){
 ...
 # Derive strong stock biomass
  ...
  # Derive equilibrium abundance of weak stock with p_s and p_w as a function of N_s and N_w catch
  N_w_star_3 <- (k_w * (c * q_w * r_s + k_s * p_s_star * q_s * (- q_w * r_s + q_s * r_w))) / 
    (k_w * p_w_star * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w)
  
  # Derive effort with p_s and p_w as a function of N_s and N_w  catch
  E_star_3 <- ((- c + k_s * p_s_star * q_s + k_w * p_w_star * q_w) * r_s * r_w)  /
   (k_w * p_w_star * q_w^2 * r_s + k_s * p_s_star * q_s^2 * r_w) 
  
  # Derive equilibrium abundance of strong stock with p_s and p_w as a function of N_s and N_w  catch
  ...
  # Build output
  output <- data.frame(E = E, N_s = N_s, N_w = N_w, p_w_star = p_w_star, p_s_star = p_s_star, C_w = C_w, C_s = C_s,
...
#write_csv(output_r_tidy, file = file.path(outdir, "output_r_ranges.csv"), col_names = T)
```
...
#write_csv(output_p_tidy, file = file.path(outdir, "output_fcn_tests.csv"), col_names = T)
```
