...
run_model_2 <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c, f_w, beta_w, A_w){
...
```{r}
scenarios_df2 <- expand.grid(p_s = 1,
                             p_w = seq(1, 10, 0.1),
                             r_s = seq(0.1, 1, 0.01),
                             r_w = seq(0.1, 0.9, 0.01),
                             q_s = 0.02,
...
for(i in 1:nrow(scenarios_df_w_fcn)){
  ...
  # Run model
  output_w_fcn <- run_model_2(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                        q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w,
