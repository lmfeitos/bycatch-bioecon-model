...
run_model_pw_fcn <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c, f_w, beta_w, A_w){
  ...
  N_s_multi2 <- (k_s * (c * q_s * r_w + k_w * P_w * q_w * (q_w * r_s - q_s * r_w))) /
    ...
    # Single species equilibrium for the weak stock 
  N_w_single2 <- c / P_w * q_w
  
    # Single species equilibrium for the strong stock  
#  N_s_single2 <- c / p_s * q_s
...
run_model_3 <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c, f_s, beta_s, A_s){
  ...
  # Weak stock catch function
  Q_s <- q_s * N_s^beta_s * E
  
    # Weak stock price function
  P_s <- (A_s * Q_s)^-f_s
  
    # Weak stock equilibrium points when prices are constants
  N_w_multi3 <- (k_w * (c * q_w * r_s + k_s * P_s * q_s * (- q_w * r_s + q_s * r_w))) / 
    (k_w * p_w * q_w^2 * r_s + k_s * P_s * q_s^2 * r_w)
  
  # Effort equilibrium point when prices are constants
  E_multi3 <- ((- c + k_s * P_s * q_s + k_w * p_w * q_w) * r_s * r_w)  /
   (k_w * p_w * q_w^2 * r_s + k_s * P_s * q_s^2 * r_w) 
 
  # Strong stock equilibrium points when prices are constants 
  N_s_multi3 <- (k_s * (c * q_s * r_w + k_w * p_w * q_w * (q_w * r_s - q_s * r_w))) /
     (k_w * p_w * q_w^2 * r_s + k_s * P_s * q_s^2 * r_w) 
  
    # Single species equilibrium for the weak stock 
  N_s_single2 <- c / P_s * q_w

    # Single species effort equilibrium for the strong stock  
  E_single2 <- (r_s / q_s * (1 - c / (P_s * q_s * k_s)))
  ...
  output <- data.frame(N_s = N_s, N_w = N_w, E = E, N_w3 = N_w3, N_s3 = N_s3, E3 = E3)
...
scenarios_df <- expand.grid(p_s = seq(1, 10, 0.2),
                            p_w = c(0.001, 1),
                            r_s = seq(0.1, 1, 0.01),
...
```{r test with weak stock price as a function of catch}
# Building df of scenarios
scenarios_df_w_fcn <- 
  expand.grid(p_s = seq(1, 5, 4), # 
              p_w = seq(1, 5, 4), #seq(1, 10, 0.1),
              r_s = seq(0.01, 1, 0.01),
...
# test scenario
r_s <- scenarios_df_w_fcn$r_s[1]
k_s <- scenarios_df_w_fcn$k_s[1]
q_s <- scenarios_df_w_fcn$q_s[1]
p_s <- scenarios_df_w_fcn$p_s[1]
r_w <- scenarios_df_w_fcn$r_w[1]
k_w <- scenarios_df_w_fcn$k_w[1]
q_w <- scenarios_df_w_fcn$q_w[1]
p_w <- scenarios_df_w_fcn$p_w[1]
f_w <- scenarios_df_w_fcn$f_w[1]
beta_w <- scenarios_df_w_fcn$beta_w[1]
A_w <- scenarios_df_w_fcn$A_w[1]
c <- scenarios_df_w_fcn$c[1]
...
for(i in 1:nrow(scenarios_df_w_fcn)){
 
  # Extract params
  r_s <- scenarios_df_w_fcn$r_s[i]
  k_s <- scenarios_df_w_fcn$k_s[i]
  q_s <- scenarios_df_w_fcn$q_s[i]
  p_s <- scenarios_df_w_fcn$p_s[i]
  r_w <- scenarios_df_w_fcn$r_w[i]
  k_w <- scenarios_df_w_fcn$k_w[i]
  q_w <- scenarios_df_w_fcn$q_w[i]
  p_w <- scenarios_df_w_fcn$p_w[i]
  f_w <- scenarios_df_w_fcn$f_w[i]
  beta_w <- scenarios_df_w_fcn$beta_w[i]
  A_w <- scenarios_df_w_fcn$A_w[i]
  c <- scenarios_df_w_fcn$c[i]
  sc_id <- scenarios_df_w_fcn$scenario_id[i]
 
  # Run model
  output_w_fcn <- run_model_pw_fcn(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                        q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w,
                        f_w = f_w, beta_w = beta_w, A_w = A_w, c = c) %>% 
    mutate(sc_id = sc_id)
  
  if(i == 1){
    output_w <- output_w_fcn
  }
  else{
    output_w <- rbind(output_w, output_w_fcn)
  }

}

output_join_test_w <- output_w %>% 
  rename(scenario_id = sc_id) %>% 
  left_join(scenarios_df_w_fcn, by = "scenario_id") %>% 
  relocate(scenario_id, .before = E) %>% 
  mutate(vuln_ratio_w = (q_w/r_w) / (q_s/r_s)) %>% 
  mutate(vuln_ratio_s = (q_s/r_s) / (q_w/r_w))

#write_csv(output_join_test_w, file = file.path(outdir, "output_w_fcn.csv"), col_names = T)
```

```{r test with strong stock price as a function of catch}
# Building df of scenarios
scenarios_df_s_fcn <- 
  expand.grid(p_s = 1,
              p_w = 1, #seq(1, 10, 0.1),
              r_s = seq(0.01, 1, 0.01),
              r_w = seq(0.01, 0.999, 0.01),
              q_s = 0.01,
              q_w = 0.01,
              k_s = 4,
              k_w = 2,
              f_s = seq(0.1, 0.5, 0.1),
              beta_s = seq(0.4, 0.8, 0.1),
              A_s = 1,
              c = 0.05) %>% 
  mutate(scenario_id = 1:nrow(.)) 

# test scenario
r_s <- scenarios_df_s_fcn$r_s[1]
k_s <- scenarios_df_s_fcn$k_s[1]
q_s <- scenarios_df_s_fcn$q_s[1]
p_s <- scenarios_df_s_fcn$p_s[1]
r_w <- scenarios_df_s_fcn$r_w[1]
k_w <- scenarios_df_s_fcn$k_w[1]
q_w <- scenarios_df_s_fcn$q_w[1]
p_w <- scenarios_df_s_fcn$p_w[1]
f_s <- scenarios_df_s_fcn$f_s[1]
beta_s <- scenarios_df_s_fcn$beta_s[1]
A_s <- scenarios_df_s_fcn$A_s[1]
c <- scenarios_df_s_fcn$c[1]
...
for(i in 1:nrow(scenarios_df_s_fcn)){
 
  # Extract params
  r_s <- scenarios_df_s_fcn$r_s[i]
  k_s <- scenarios_df_s_fcn$k_s[i]
  q_s <- scenarios_df_s_fcn$q_s[i]
  p_s <- scenarios_df_s_fcn$p_s[i]
  r_w <- scenarios_df_s_fcn$r_w[i]
  k_w <- scenarios_df_s_fcn$k_w[i]
  q_w <- scenarios_df_s_fcn$q_w[i]
  p_w <- scenarios_df_s_fcn$p_w[i]
  f_s <- scenarios_df_s_fcn$f_s[i]
  beta_s <- scenarios_df_s_fcn$beta_s[i]
  A_s <- scenarios_df_s_fcn$A_s[i]
  c <- scenarios_df_s_fcn$c[i]
  sc_id <- scenarios_df_s_fcn$scenario_id[i]
 
  # Run model
  output_s_fcn <- run_model_3(r_s = r_s, r_w = r_w, k_s = k_s, k_w = k_w,
                        q_s = q_s, q_w = q_w, p_s = p_s, p_w = p_w,
                        f_s = f_s, beta_s = beta_s, A_s = A_s, c = c) %>% 
    mutate(sc_id = sc_id)
  
  if(i == 1){
    output_s <- output_s_fcn
  }
  else{
    output_s <- rbind(output_s, output_s_fcn)
  }

}

output_join_test_s <- output_s %>% 
  rename(scenario_id = sc_id) %>% 
  left_join(scenarios_df_s_fcn, by = "scenario_id") %>% 
  relocate(scenario_id, .before = E) %>% 
  mutate(vuln_ratio_w = (q_w/r_w) / (q_s/r_s)) %>% 
  mutate(vuln_ratio_s = (q_s/r_s) / (q_w/r_w))

#write_csv(output_join_test_w, file = file.path(outdir, "output_s_fcn.csv"), col_names = T)
```
