...
run_model_2 <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c, f_s, f_w, beta_s, beta_w, alpha, A){
  ...
  dN_s <- (r_s * N_s * (1 - N_s/k_s) - q_s * (N_s ^ beta_s) * E) * dt
  
  dN_w <- (r_w * N_w * (1 - N_w/k_w) - q_w * (N_w ^ beta_w) * E) * dt
  
  dE <- (alpha * E * ((A * (q_s * (N_s^beta_s) * E)^-f_s)* q_s * (N_s^beta_s) + (A * (q_w * (N_w^beta_w) * E)^-f_w) * q_w * (N_w^beta_w)) - c) * dt
  
  output <- data.frame(N_s = N_s, N_w = N_w, E = E, )
