...
E_simu8_s <- (alpha * E8 * (A_s * (q_s * N_s8^beta_s8 * E8)^-f_s8 * q_s * N_s8^beta_s8 - (c))) * dt
```
...
#                names_to = "variable",
#                values_to = "value") %>% 
#   mutate(scenario_id = "5") cost_ns_df, cost_nw_df, cost_sum_df, rev_ns_df, rev_nw_df, rev_sum_df

# Gathered the results from each model scenario run into a single df

var_full_price <- bind_rows(effort_df, nw_df, ns_df, catch_s_df, catch_w_df, cpue_s_df, cpue_w_df, profits_df,
                            profits_w_df, profits_s_df)
