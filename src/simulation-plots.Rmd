...
output_price %>% 
  ...
  ggplot() +
  geom_col(aes(x = prices, y = values, fill = variables),
           position = "dodge") +
...
output_gp <- output_price %>% 
  ...
  left_join(output_q, by = c("scenario_id", "variables", "values"))
...
```{r}
ggplot(data = output_gp %>% filter(variables %in% c("N_w", "N_s"))) +
  geom_point(aes(x = price_ratio, y = q_ratio, color = variables),
             alpha = 0.5) +
  labs(x = "Price ratio (s:w)",
       y = "Productivity ratio (s:w)",
       title = "Simulation of the distribution of catchability and price ratios") +
  theme_bw()
```
