...
# function for the differential equations
test_fcn <- function(time, state_vars, param){
  with(as.list(c(state_vars, param)),{
  # differential equations
  dN_s1 <- (N_s1 * r_s * (1 - (N_s1/k_s)) - q_s * N_s1^beta_s1 * E)
  dN_w1 <- (N_w1 * r_w * (1 - (N_w1/k_w)) - q_w * N_w1^beta_w1 * E)
  dE <- (alpha * E * (A_s * (q_s * N_s1^beta_s1 * E)^-f_s1 * q_s * N_s1^beta_s1 + A_w * (q_w * N_w1^beta_w1 * E)^-f_w1 * q_w * N_w1^beta_w1) - c)
  dC_s1 <- (q_s * N_s1 * E)
  dC_w1 <- (q_w * N_w1 * E)
  dcpue_s1 <- (C_s1 / E)
  dcpue_w1 <- (C_w1 / E)
  dPi1 <- (C_s1 * A_s * (q_s * N_s1^beta_s1 * E)^-f_s1 + C_w1 * A_w * (q_w * N_w1^beta_w1 * E)^-f_w1 - ((C_s1 + C_w1) * c))
  dPi_s1 <- (C_s1 * A_s * (q_s * N_s1^beta_s1 * E)^-f_s1 - C_s1 * c)
  dPi_w1 <- (C_w1 * A_w * (q_w * N_w1^beta_w1 * E)^-f_w1 - C_w1 * c)

  
  # return of rate of change
  list(c(dN_s1, dN_w1, dE, dC_s1, dC_w1, dcpue_s1, dcpue_w1, dPi1, dPi_s1, dPi_w1))
 })
}
...
out <- ode(y = state_vars, times = time, func = test_fcn, parms = param)
tail(out)

out_df <- bind_rows(lapply(out, as.data.frame), .id = "time")
```
