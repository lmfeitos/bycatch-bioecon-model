...
for(i in 2:length(tset)) {
  ...
  if(tset[i] < 160){ N_w_simu1 <- N_w_simu1} else {N_w_simu1 <- 0}
  if(tset[i] < 160){ N_s_simu1 <- N_s_simu1} else {N_s_simu1 <- (N_s1 * r_s * (1 - (N_s1/k_s)) - q_s * N_s1^beta_s1 * E) * dt}
  if(tset[i] < 160){ E_simu1 <- E_simu1} else {E_simu1 <- (alpha * E * (A_s * (q_s * N_s1^beta_s1 * E)^-f_s1 * q_s * N_s1^beta_s1) - c) * dt}
  if(tset[i] < 160){ Pi_sum_simu1 <- Pi_sum_simu1} else {Pi_sum_simu1 <- Pi_s_simu1}
  ...
  dC_s1 <- (q_s * N_s1^beta_s1 * E) * dt
  dC_w1 <- (q_w * N_w1^beta_w1 * E) * dt
  dcpue_s1 <- (C_s1 / E) * dt
  dcpue_w1 <- (C_w1 / E) * dt
...
```{r, include = FALSE}
# run_model <- function(r_s, k_s, q_s, p_s, r_w, k_w, q_w, p_w, c, beta_s, beta_w, f_w, f_s, A_s, A_w){
#   
#   # Derive effort
#   E <- ((- c + k_s * p_s * q_s + k_w * p_w * q_w) * r_s * r_w)  /
#    (k_w * p_w * q_w^2 * r_s + k_s * p_s * q_s^2 * r_w) 
...
```{r, echo = FALSE}
effort_df <- cbind(E_simu1, E_simu2, E_simu3, E_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("E_"),
               names_to = "variable",
               values_to = "value") %>% 
...
nw_df <- cbind(N_w_simu1, N_w_simu2, N_w_simu3, N_w_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("N_w_simu"),
               names_to = "variable",
               values_to = "value") %>% 
...
ns_df <- cbind(N_s_simu1,N_s_simu2, N_s_simu3, N_s_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("N_s_simu"),
               names_to = "variable",
               values_to = "value") %>% 
...
catch_s_df <- cbind(C_s_simu1,C_s_simu2, C_s_simu3, C_s_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("C_s_"),
               names_to = "variable",
               values_to = "value") %>% 
...
catch_w_df <- cbind(C_w_simu1,C_w_simu2, C_w_simu3, C_w_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("C_w_"),
               names_to = "variable",
               values_to = "value") %>% 
...
cpue_s_df <- cbind(cpue_s_simu1, cpue_s_simu2, cpue_s_simu3, cpue_s_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("cpue_s_"),
               names_to = "variable",
               values_to = "value") %>% 
...
# cpue_s_simu5, cpue_s_simu6,

cpue_w_df <- cbind(cpue_w_simu1, cpue_w_simu2, cpue_w_simu3, cpue_w_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("cpue_w_"),
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = case_when(
    ...
    str_detect(variable, "3") ~ "3",
    str_detect(variable, "4") ~ "4"))
# cpue_w_simu5, cpue_w_simu6,

profits_df <- cbind(Pi_sum_simu1, Pi_sum_simu2, Pi_sum_simu3, Pi_sum_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_sum"),
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = case_when(
    ...
    str_detect(variable, "3") ~ "3",
    str_detect(variable, "4") ~ "4"))
# Pi_sum_simu5, Pi_sum_simu6,

profits_s_df <- cbind(Pi_s_simu1, Pi_s_simu2, Pi_s_simu3, Pi_s_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_s_"),
               names_to = "variable",
               values_to = "value") %>% 
...
# Pi_s_simu5, Pi_s_simu6,

profits_w_df <- cbind(Pi_w_simu1, Pi_w_simu2, Pi_w_simu3, Pi_w_simu4, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_w_"),
               names_to = "variable",
               values_to = "value") %>% 
...
var_plot_price <- var_full_price %>%
   mutate(f_ratio = case_when(
     str_detect(scenario_id, "1") ~ "f_w/f_s = 0.6",
     str_detect(scenario_id, "2") ~ "f_w/f_s = 0.16",
     str_detect(scenario_id, "3") ~ "f_w/f_s = 1",
     str_detect(scenario_id, "4") ~ "f_w/f_s = 2",
   )) %>% 
   mutate(beta_ratio = case_when(
     str_detect(scenario_id, "1") ~ "beta_w/beta_s = 1.2",
     str_detect(scenario_id, "2") ~ "beta_w/beta_s = 1.6",
     str_detect(scenario_id, "3") ~ "beta_w/beta_s = 0.8",
     str_detect(scenario_id, "4") ~ "beta_w/beta_s = 0.8",
   )) %>% 
  mutate(variable_clean = case_when(
    str_detect(variable, "N_w") ~ "Weak stock",
    str_detect(variable, "N_s") ~ "Strong stock",
