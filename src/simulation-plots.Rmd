...
# Create facet labels
sub1_label1 <- c("10% subsidy")
names(sub1_label1) <- c("0.045")

sub1_label2 <- c("25% subsidy")
names(sub1_label2) <- c("0.0375")

sub1_label3 <- c("50% subsidy")
...
subs1_s_proc_plot1.1 <-
  ...
  geom_tile(aes(x = round(vuln_ratio_w, 8), y = round(st_stock_rev, 8), fill = b_v_k_w, height = 1, width = .1)) +
  scale_fill_paletteer_c("grDevices::Blues 3",
                         direction = -1,
                         limits = col_limit) +
  facet_wrap(~ c,
...
subs1_s_proc_plot3.1 <-
  ...
  geom_tile(aes(x = round(vuln_ratio_w, 8), y = round(st_stock_rev, 8), fill = b_v_k_w, height = 1.6, width = .1)) +
  scale_fill_paletteer_c("grDevices::Blues 3",
                         direction = -1,
                         limits = col_limit) +
  facet_wrap(~ c,
...
test <- output_subs1_proc %>% 
  mutate(extinct = case_when(
    b_v_k_w == 0 ~ "yes",
    b_v_k_w > 0 ~ "no",
  )) %>% 
  filter(p_w == 1 & c == 0.025 & extinct == "yes")
...
subs1_w_proc_plot1.1 <-
  ...
  geom_tile(aes(x = round(vuln_ratio_w, 8), y = round(w_stock_rev, 8), fill = b_v_k_w, height = 1, width = .1)) +
  scale_fill_paletteer_c("grDevices::Blues 3",
                         direction = -1,
                         limits = col_limit) +
  facet_wrap(~ c,
...
subs1_w_proc_plot3.1 <-
  ...
  geom_tile(aes(x = round(vuln_ratio_w, 8), y = round(w_stock_rev, 8), fill = b_v_k_w, height = 1.6, width = .1)) +
  scale_fill_paletteer_c("grDevices::Blues 3",
                         direction = -1,
                         limits = col_limit) +
  facet_wrap(~ c,
...
sub_w1.1 <- grid.arrange(patchworkGrob(subs1_w_proc_plot0.1 /
  ...
  bottom = "Vulnerability ratios (Vw/Vs)")  
...
C_s_simu1 <- NaN*tset; C_s_simu1[1] <- q_s * k_s^beta_s1 * E

C_w_simu1 <- NaN*tset; C_w_simu1[1] <- q_w * k_w^beta_w1 * E
...
# change in time
  ...
  N_s_simu1[i] <- N_s1 + dN_s1
  E_simu1[i] <- E1 + dE1
  C_s_simu1[i] <- q_s * (N_s1 + dN_s1)^beta_s1 * E1 + dE1
  C_w_simu1[i] <- q_w * (N_w1 + dN_w1)^beta_w1 * E1 + dE1
  cpue_s_simu1[i] <- (q_s * (N_s1 + dN_s1)^beta_s1 * E1 + dE1) / (E1 + dE1)
  cpue_w_simu1[i] <- (q_w * (N_w1 + dN_w1)^beta_w1 * E1 + dE1) / (E1 + dE1)
  Pi_sum_simu1[i] <- sum(pi_s1 + dPi_s1 + pi_w1 + dPi_w1)
  Pi_s_simu1[i] <- (C_s1 * A_s * (q_s * N_s1^beta_s1 * E1)^-f_s1 - C_s1 * c) + dPi_s1
  Pi_w_simu1[i] <- (C_w1 * A_w * (q_w * N_w1^beta_w1 * E1)^-f_w1 - C_w1 * c) + dPi_w1  
...
```{r}
plot(x = tset, y = C_s_simu1,
     col = "black")
lines(x = tset, y = C_w_simu1,
      col = "blue")
lines(x = tset, y = cpue_s_simu1,
      col = "green")
lines(x = tset, y = cpue_w_simu1,
      col = "red")
```
...
C_s_simu2 <- NaN*tset; C_s_simu2[1] <- q_s * k_s^beta_s2 * E

C_w_simu2 <- NaN*tset; C_w_simu2[1] <- q_w * k_w^beta_w2 * E
...
#  dPi2 <- (C_s2 * A_s * (q_s * N_s2^beta_s2 * E2)^-f_s2 + C_w2 * A_w * (q_w * N_w2^beta_w2 * E2)^-f_w2 - ((C_s2 + C_w2) * c)) * dt
  ...
  N_s_simu2[i] <- N_s2 + dN_s2
  E_simu2[i] <- E2 + dE2
  C_s_simu2[i] <- q_s * (N_s2 + dN_s2)^beta_s2 * E2 + dE2
  C_w_simu2[i] <- q_w * (N_w2 + dN_w2)^beta_w2 * E2 + dE2
  cpue_s_simu2[i] <- (q_s * (N_s2 + dN_s2)^beta_s2 * E2 + dE2) / (E2 + dE2)
  cpue_w_simu2[i] <- (q_w * (N_w2 + dN_w2)^beta_w2 * E2 + dE2) / (E2 + dE2)
  Pi_sum_simu2[i] <- sum(pi_s2 + dPi_s2 + pi_w2 + dPi_w2)
  Pi_s_simu2[i] <- (C_s2 * A_s * (q_s * N_s2^beta_s2 * E2)^-f_s2 - C_s2 * c) + dPi_s2
  Pi_w_simu2[i] <- (C_w2 * A_w * (q_w * N_w2^beta_w2 * E2)^-f_w2 - C_w2 * c) + dPi_w2  
...
C_s_simu3 <- NaN*tset; C_s_simu3[1] <- q_s * k_s^beta_s3 * E

C_w_simu3 <- NaN*tset; C_w_simu3[1] <- q_w * k_w^beta_w3 * E

cpue_s_simu3 <- NaN*tset; cpue_s_simu3[1] <- 0

cpue_w_simu3 <- NaN*tset; cpue_w_simu3[1] <- 0

Pi_sum_simu3 <- NaN*tset; Pi_sum_simu3[1] <- 0

Pi_s_simu3 <- NaN*tset; Pi_s_simu3[1] <- 0

Pi_w_simu3 <- NaN*tset; Pi_w_simu3[1] <- 0

cost_ns_simu3 <- NaN*tset; cost_ns_simu3[1] <- 0.05

cost_nw_simu3 <- NaN*tset; cost_nw_simu3[1] <- 0.05

cost_sum_simu3 <- NaN*tset; cost_sum_simu3[1] <- 0.1

rev_ns_simu3 <- NaN*tset; rev_ns_simu3[1] <- 0

rev_nw_simu3 <- NaN*tset; rev_nw_simu3[1] <- 0

rev_sum_simu3 <- NaN*tset; rev_sum_simu3[1] <- 0
...
for(i in 2:length(tset)) {
  ...
  pi_s3 <- Pi_s_simu3[i-1]
  pi_w3 <- Pi_w_simu3[i-1]
  cost_ns3 <- cost_ns_simu3[i-1]
  cost_nw3 <- cost_nw_simu3[i-1]
  cost_sum3 <- cost_sum_simu3[i-1]
  rev_ns3 <- rev_ns_simu3[i-1]
  rev_nw3 <- rev_nw_simu3[i-1]
  rev_sum3 <- rev_sum_simu3[i-1]
  ...
  # Calculate change in variable size at each time step
    ...
    dcost_ns3 <- (c * E3/C_s3) * dt
    dcost_nw3 <- (c * E3/C_w3) * dt
    #dcost_sum3 <- (E3 * (cost_nw3 + cost_ns3)/(C_s3 + C_w3)) * dt
    drev_ns3 <- (A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 * q_s * N_s3^beta_s3 * E3/C_s3) * dt
    drev_nw3 <- (A_w * (q_w * N_w3^beta_w3 * E3)^-f_w3 * q_s * N_w3^beta_w3 * E3/C_w3) * dt
    #drev_sum3 <- ((A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 * q_s * N_s3^beta_s3 * E3 + 
    #                 A_w * (q_w * N_w3^beta_w3 * E3)^-f_w3 * q_w * N_w3^beta_w3 * E3/(C_s3 + C_w3))) * dt
  
  } else {
    # If N_w3 < 0, set each variable dependent on w to zero and calculate all the other ones only with the strong stock.
    dN_s3 <- (r_s * N_s3 * (1 - (N_s3/k_s)) - (q_s * N_s3^beta_s3 * E3)) * dt
    dN_w3 <- 0
    dE3 <- (alpha * E3 * (A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 * q_s * N_s3^beta_s3 - (c))) * dt
    dC_s3 <- (q_s * N_s3^beta_s3 * E3) * dt
    dC_w3 <- 0
    #dcpue_s3 <- (C_s3 / E3) * dt
    #dcpue_w3 <- 0
    dPi_w3 <- 0
    dPi_s3 <- (C_s3 * A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 - (C_s3 * c)) * dt
    #dPi3 <- (C_s3 * A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 - (C_s3 * c)) * dt
    dcost_ns3 <- (c * E3/C_s3) * dt
    dcost_nw3 <- 0
    #dcost_sum3 <- ((E3 * cost_ns3)/C_s3) * dt
    drev_nw3 <- 0
    drev_ns3 <- (A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 * q_s * N_s3^beta_s3 * E3/C_s3) * dt
    #drev_sum3 <- (A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 * q_s * N_s3^beta_s3 * E3/C_s3) * dt
  }
  
  # calculate total variable size and store in holding vectors
  N_w_simu3[i] <- ifelse(N_w3 + dN_w3 < 0, 0, N_w3 + dN_w3)
  N_s_simu3[i] <- N_s3 + dN_s3
  E_simu3[i] <- E3 + dE3
  C_s_simu3[i] <- q_s * (N_s3 + dN_s3)^beta_s3 * (E3 + dE3)
  C_w_simu3[i] <- ifelse(N_w3 + dN_w3 < 0, 0, q_w * (N_w3 + dN_w3)^beta_w3 * (E3 + dE3))
  cpue_s_simu3[i] <- (q_s * (N_s3 + dN_s3)^beta_s3 * E3 + dE3) / (E3 + dE3)
  cpue_w_simu3[i] <- (q_w * (N_w3 + dN_w3)^beta_w3 * E3 + dE3) / (E3 + dE3)
  Pi_sum_simu3[i] <- sum(pi_s3 + dPi_s3 + ifelse(N_w3 + dN_w3 < 0, 0, pi_w3 + dPi_w3))
  Pi_s_simu3[i] <- (q_s * (N_s3 + dN_s3)^beta_s3 * (E3 + dE3) * A_s * (q_s * N_s3^beta_s3 * E3)^-f_s3 - (q_s * (N_s3 + dN_s3)^beta_s3 * (E3 + dE3) * c)) + dPi_s3
  Pi_w_simu3[i] <- ifelse(N_w3 + dN_w3 < 0, 0, q_w * (N_w3 + dN_w3)^beta_w3 * (E3 + dE3) * A_w * (q_w * N_w3^beta_w3 * E3)^-f_w3 
                                                                            - (q_w * (N_w3 + dN_w3)^beta_w3 * (E3 + dE3) * c))  + dPi_w3
  cost_ns_simu3[i] <- cost_ns3 + dcost_ns3
  cost_nw_simu3[i] <- ifelse(N_w3 + dN_w3 < 0, 0, cost_nw3 + dcost_nw3)
  cost_sum_simu3[i] <- sum(cost_ns3 + dcost_ns3 + ifelse(N_w3 + dN_w3 < 0, 0, cost_nw3 + dcost_nw3))
  rev_ns_simu3[i] <- rev_ns3 + drev_ns3
  rev_nw_simu3[i] <- ifelse(N_w3 + dN_w3 < 0, 0, rev_nw3 + drev_nw3)
  rev_sum_simu3[i] <- sum(rev_ns3 + drev_ns3 + ifelse(N_w3 + dN_w3 < 0, 0, rev_nw3 + drev_nw3))
}
...
C_s_simu4 <- NaN*tset; C_s_simu4[1] <- q_s * k_s^beta_s4 * E

C_w_simu4 <- NaN*tset; C_w_simu4[1] <- q_w * k_w^beta_w4 * E

cpue_s_simu4 <- NaN*tset; cpue_s_simu4[1] <- 0

cpue_w_simu4 <- NaN*tset; cpue_w_simu4[1] <- 0

Pi_sum_simu4 <- NaN*tset; Pi_sum_simu4[1] <- 0

Pi_s_simu4 <- NaN*tset; Pi_s_simu4[1] <- 0

Pi_w_simu4 <- NaN*tset; Pi_w_simu4[1] <- 0

cost_ns_simu4 <- NaN*tset; cost_ns_simu4[1] <- 0.05

cost_nw_simu4 <- NaN*tset; cost_nw_simu4[1] <- 0.05

cost_sum_simu4 <- NaN*tset; cost_sum_simu4[1] <- 0.1

rev_ns_simu4 <- NaN*tset; rev_ns_simu4[1] <- 0

rev_nw_simu4 <- NaN*tset; rev_nw_simu4[1] <- 0

rev_sum_simu4 <- NaN*tset; rev_sum_simu4[1] <- 0
...
for(i in 2:length(tset)) {
  ...
  pi_s4 <- Pi_s_simu4[i-1]
  pi_w4 <- Pi_w_simu4[i-1]
  cost_ns4 <- cost_ns_simu4[i-1]
  cost_nw4 <- cost_nw_simu4[i-1]
  cost_sum4 <- cost_sum_simu4[i-1]
  rev_ns4 <- rev_ns_simu4[i-1]
  rev_nw4 <- rev_nw_simu4[i-1]
  rev_sum4 <- rev_sum_simu4[i-1]
  ...
  # Calculate change in variable size at each time step
    ...
    dcost_ns4 <- (c * E4/C_s4) * dt
    dcost_nw4 <- (c * E4/C_w4) * dt
    #dcost_sum4 <- (E4 * (cost_nw4 + cost_ns4)/(C_s4 + C_w4)) * dt
    drev_ns4 <- (A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 * q_s * N_s4^beta_s4 * E4/C_s4) * dt
    drev_nw4 <- (A_w * (q_w * N_w4^beta_w4 * E4)^-f_w4 * q_s * N_w4^beta_w4 * E4/C_w4) * dt
    #drev_sum4 <- ((A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 * q_s * N_s4^beta_s4 * E4 + 
    #                 A_w * (q_w * N_w4^beta_w4 * E4)^-f_w4 * q_w * N_w4^beta_w4 * E4/(C_s4 + C_w4))) * dt
  
  } else {
    # If N_w4 < 0, set each variable dependent on w to zero and calculate all the other ones only with the strong stock.
    dN_s4 <- (r_s * N_s4 * (1 - (N_s4/k_s)) - (q_s * N_s4^beta_s4 * E4)) * dt
    dN_w4 <- 0
    dE4 <- (alpha * E4 * (A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 * q_s * N_s4^beta_s4 - (c))) * dt
    dC_s4 <- (q_s * N_s4^beta_s4 * E4) * dt
    dC_w4 <- 0
    dPi_w4 <- 0
    dPi_s4 <- (C_s4 * A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 - (C_s4 * c)) * dt
    #dPi4 <- (C_s4 * A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 - (C_s4 * c)) * dt
    dcost_ns4 <- (c * E4/C_s4) * dt
    dcost_nw4 <- 0
    #dcost_sum4 <- ((E4 * cost_ns4)/C_s4) * dt
    drev_nw4 <- 0
    drev_ns4 <- (A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 * q_s * N_s4^beta_s4 * E4/C_s4) * dt
    #drev_sum4 <- (A_s * (q_s * N_s4^beta_s4 * E4)^-f_s4 * q_s * N_s4^beta_s4 * E4/C_s4) * dt
  }
  
  # calculate total variable size and store in holding vectors
  
  N_w_simu4[i] <- ifelse(N_w4 + dN_w4 < 0, 0, N_w4 + dN_w4)
  N_s_simu4[i] <- N_s4 + dN_s4
  E_simu4[i] <- E4 + dE4
  C_s_simu4[i] <- q_s * (N_s4 + dN_s4)^beta_s4 * E4 + dE4
  C_w_simu4[i] <- ifelse(N_w4 + dN_w4 < 0, 0, q_w * (N_w4 + dN_w4)^beta_w4 * E4 + dE4)
  cpue_s_simu4[i] <- (q_s * (N_s4 + dN_s4)^beta_s4 * E4 + dE4) / (E4 + dE4)
  cpue_w_simu4[i] <- (q_w * (N_w4 + dN_w4)^beta_w4 * E4 + dE4) / (E4 + dE4)
  Pi_sum_simu4[i] <- sum(pi_s4 + dPi_s4 + ifelse(N_w4 + dN_w4 < 0, 0, pi_w4 + dPi_w4))
  Pi_s_simu4[i] <- pi_s4 + dPi_s4
  Pi_w_simu4[i] <- ifelse(N_w4 + dN_w4 < 0, 0, pi_w4 + dPi_w4)
  cost_ns_simu4[i] <- cost_ns4 + dcost_ns4
  cost_nw_simu4[i] <- ifelse(N_w4 + dN_w4 < 0, 0, cost_nw4 + dcost_nw4)
  cost_sum_simu4[i] <- sum(cost_ns4 + dcost_ns4 + ifelse(N_w4 + dN_w4 < 0, 0, cost_nw4 + dcost_nw4))
  rev_ns_simu4[i] <- rev_ns4 + drev_ns4
  rev_nw_simu4[i] <- ifelse(N_w4 + dN_w4 < 0, 0, rev_nw4 + drev_nw4)
  rev_sum_simu4[i] <- sum(rev_ns4 + drev_ns4 + ifelse(N_w4 + dN_w4 < 0, 0, rev_nw4 + drev_nw4))
}
...
C_s_simu5 <- NaN*tset; C_s_simu5[1] <- q_s * k_s^beta_s5 * E

C_w_simu5 <- NaN*tset; C_w_simu5[1] <- q_w * k_w^beta_w5 * E
...
for(i in 2:length(tset)) {
  ...
  # Calculate change in variable size at each time step
  # If N_w5 > 0, calculate each variable for each time step like the equations below:
  if(N_w5 > 0) {
    ...
    #dPi5 <- (C_s5 * A_s * (q_s * N_s5^beta_s5 * E5)^-f_s5 + 
    #           C_w5 * A_w * (q_w * N_w5^beta_w5 * E5)^-f_w5 - ((C_s5 + C_w5) * c)) * dt
  ...
  N_s_simu5[i] <- N_s5 + dN_s5
  E_simu5[i] <- E5 + dE5
  C_s_simu5[i] <- q_s * (N_s5 + dN_s5)^beta_s5 * E5 + dE5
  C_w_simu5[i] <- ifelse(N_w5 + dN_w5 < 0, 0, q_w * (N_w5 + dN_w5)^beta_w5 * E5 + dE5)
  cpue_s_simu5[i] <- (q_s * (N_s5 + dN_s5)^beta_s5 * E5 + dE5) / (E5 + dE5)
  cpue_w_simu5[i] <- (q_w * (N_w5 + dN_w5)^beta_w5 * E5 + dE5) / (E5 + dE5)
  Pi_sum_simu5[i] <- sum(pi_s5 + dPi_s5 + ifelse(N_w5 + dN_w5 < 0, 0, pi_w5 + dPi_w5))
  Pi_s_simu5[i] <- pi_s5 + dPi_s5
  ...
  cost_sum_simu5[i] <- sum(cost_ns5 + dcost_ns5 + ifelse(N_w5 + dN_w5 < 0, 0, cost_nw5 + dcost_nw5))
  rev_ns_simu5[i] <- rev_ns5 + drev_ns5
  rev_nw_simu5[i] <- ifelse(N_w5 + dN_w5 < 0, 0, rev_nw5 + drev_nw5)
  rev_sum_simu5[i] <- sum(rev_ns5 + drev_ns5 + ifelse(N_w5 + dN_w5 < 0, 0, rev_nw5 + drev_nw5))
...
```{r, include = FALSE}
plot(x = log(N_s_simu5), y = log(cost_ns_simu5),
     col = "purple", las = 1,
     type = "l",
     lwd = 2,
     xlab = "Strong stock abundance",
     ylab = "Strong stock price, cost")
lines(x = log(N_s_simu5), y = rev_sum_simu5,
     col = "black", type = "l",
     lwd = 2)
legend(x = 3.5, y = 1.5e+74,
       legend = c("Revenue", "Cost"),
...
```{r, echo = FALSE}
effort_df <- cbind(E_simu1, E_simu2,E_simu3, E_simu4, E_simu5, tset) %>% #E_simu3, E_simu4, E_simu6
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("E_"),
               names_to = "variable",
               values_to = "value") %>% 
...
nw_df <- cbind(N_w_simu1, N_w_simu2, N_w_simu3, N_w_simu4, N_w_simu5, tset) %>% # N_w_simu3, N_w_simu4, N_w_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("N_w_simu"),
               names_to = "variable",
               values_to = "value") %>% 
...
ns_df <- cbind(N_s_simu1,N_s_simu2, N_s_simu3, N_s_simu4, N_s_simu5, tset) %>% # N_s_simu3, N_s_simu4, N_s_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("N_s_simu"),
               names_to = "variable",
               values_to = "value") %>% 
...
catch_s_df <- cbind(C_s_simu1,C_s_simu2, C_s_simu3, C_s_simu4, C_s_simu5, tset) %>% #C_s_simu3, C_s_simu4, C_s_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("C_s_"),
               names_to = "variable",
               values_to = "value") %>% 
...
catch_w_df <- cbind(C_w_simu1,C_w_simu2, C_w_simu3, C_w_simu4, C_w_simu5, tset) %>% #C_w_simu3, C_w_simu4,  C_w_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("C_w_"),
               names_to = "variable",
               values_to = "value") %>% 
...
cpue_s_df <- cbind(cpue_s_simu1, cpue_s_simu2, cpue_s_simu3, cpue_s_simu4, cpue_s_simu5,tset) %>% #cpue_s_simu3, cpue_s_simu4,cpue_s_simu6,
 as.data.frame() %>% 
 pivot_longer(cols = starts_with("cpue_s_"),
              names_to = "variable",
              values_to = "value") %>% 
...
cpue_w_df <- cbind(cpue_w_simu1, cpue_w_simu2, cpue_w_simu3, cpue_w_simu4, cpue_w_simu5, tset) %>% #cpue_w_simu3, cpue_w_simu4, cpue_w_simu6, 
 as.data.frame() %>% 
 pivot_longer(cols = starts_with("cpue_w_"),
              names_to = "variable",
              values_to = "value") %>% 
...
profits_df <- cbind(Pi_sum_simu1, Pi_sum_simu2, Pi_sum_simu3, Pi_sum_simu4, Pi_sum_simu5, tset) %>% #Pi_sum_simu3, Pi_sum_simu4, Pi_sum_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_sum"),
               names_to = "variable",
               values_to = "value") %>% 
...
profits_s_df <- cbind(Pi_s_simu1, Pi_s_simu2, Pi_s_simu3, Pi_s_simu4, Pi_s_simu5, tset) %>% # Pi_s_simu3, Pi_s_simu4,  Pi_s_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_s_"),
               names_to = "variable",
               values_to = "value") %>% 
...
profits_w_df <- cbind(Pi_w_simu1, Pi_w_simu2,  Pi_w_simu3, Pi_w_simu4, Pi_w_simu5, tset) %>% # Pi_w_simu3, Pi_w_simu4,Pi_w_simu6,
  as.data.frame() %>% 
  pivot_longer(cols = starts_with("Pi_w_"),
               names_to = "variable",
               values_to = "value") %>% 
 ...
 cost_ns_df <- cbind(cost_ns_simu3, cost_ns_simu4, cost_ns_simu5, tset) %>% 
   as.data.frame() %>%
 ...
 cost_nw_df <- cbind(cost_nw_simu3, cost_nw_simu4, cost_nw_simu5, tset) %>% 
   as.data.frame() %>% 
 ...
 cost_sum_df <- cbind(cost_sum_simu3, cost_sum_simu4, cost_sum_simu5, tset) %>% 
   as.data.frame() %>% 
 ...
 rev_ns_df <- cbind(rev_ns_simu3, rev_ns_simu4, rev_ns_simu5, tset) %>%  
   as.data.frame() %>% 
 ...
 rev_nw_df <- cbind(rev_nw_simu3, rev_nw_simu4, rev_nw_simu5, tset) %>%  
   as.data.frame() %>%
 ...
 rev_sum_df <- cbind(rev_sum_simu3, rev_sum_simu4, rev_sum_simu5, tset) %>%  
   as.data.frame() %>% 
...
equal_scales_catch_fcn <-
  ...
  mutate(variable_clean = str_replace_all(variable_clean, "catch", "")) %>% 
  ggplot() +
  geom_line(aes(x = tset, y = value, color = variable_clean),
            size = .8,
            alpha = .8,
...
equal_scales_cpue_fcn <-
  ...
  mutate(variable_clean = str_replace_all(variable_clean, "CPUE", "")) %>% 
  ggplot() +
  geom_line(aes(x = tset, y = value, color = variable_clean),
            size = .8,
            alpha = .8,
...
```{r, fig.height=14, fig.width=8, echo = FALSE, warning=FALSE}
fcn_plot <-
  equal_scales_EWS_fcn /
   stock_size_fcn /
  b_v_k_fcn /
...
#ggplotly(fcn_plot)
#ggsave(file.path(outdir, "profit_plot_fcn.png"), plot = equal_scales_pi_fcn,
#      height = 4, width = 6, bg = "white", dpi = 300)
```
