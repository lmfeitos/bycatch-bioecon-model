...
s_w_range_proc %>% 
  filter(p_w == 1.1) %>% 
  ggplot() + 
  geom_raster(aes(x = round(vuln_ratio_w, 3), y = round(st_stock_rev, 3), fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
  facet_wrap(~ b_stocks,
             labeller = labeller(b_stocks = b_labs)) +
...
s_w_range_proc_plot2 <-
  ...
  filter(p_w == 1.1) %>% 
  ggplot() + 
  geom_raster(aes(x =  round(vuln_ratio_s, 3), y = round(st_stock_rev, 3), fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
    facet_wrap(~ b_stocks,
               labeller = labeller(b_stocks = b_labs)) +
...
s_w_range_proc_plot3 <-
  ...
  filter(p_s == 1) %>% 
 ggplot() + 
  geom_tile(aes(x = round(vuln_ratio_w, 3), y = round(w_stock_rev, 3), fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
    facet_wrap(~ b_stocks,
               labeller = labeller(b_stocks = b_labs)) +
...
s_w_range_proc_plot4 <-
 s_w_range_proc %>% 
  filter(p_s == 1) %>%  
  ggplot() + 
  geom_tile(aes(x = round(vuln_ratio_s, 3), y = round(w_stock_rev, 3), fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
   facet_wrap(~ b_stocks,
                labeller = labeller(b_stocks = b_labs)) +
...
s_w_range_proc %>% 
  ggplot() +
  geom_histogram(aes(x = st_stock_rev))
```
...
w_s_range_proc %>% 
  filter(p_w == 1) %>% 
  ggplot() + 
  geom_raster(aes(x = round(vuln_ratio_w, 3), y = st_stock_rev, fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
  facet_wrap(~ b_stocks,
             labeller = labeller(b_stocks = b_labs)) +
...
w_s_range_proc %>% 
  filter(p_w == 1) %>% 
  ggplot() + 
  geom_tile(aes(x = round(vuln_ratio_w, 10), y = w_stock_rev, fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
  facet_wrap(~ b_stocks,
             labeller = labeller(b_stocks = b_labs)) +
...
w_s_range_proc %>% 
  filter(p_w == 1) %>% 
  ggplot() +
  geom_histogram(aes(x = r_s))
```

```{r, echo = FALSE}
w_s_zero_proc %>% 
  filter(r_s > r_w) %>% 
  filter(variables == "N_w") %>% 
  ggplot() + 
  geom_raster(aes(x = round(r_w/r_s, 3), y = w_stock_rev, fill = round(b_v_k, 3))) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5)) +
  scale_x_continuous(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0)) +
...
w_s_zero_proc %>% 
  ggplot() +
  geom_histogram(aes(x = r_s/r_w, fill = variables),
                 position = "dodge")
```

```{r, echo = FALSE}
p_w_labs2 <- c("pw = 1", "pw = 10", "pw = 50", "pw = 100")
names(p_w_labs2) <- c(1, 10, 50, 100)

s_w_range_proc_subset2 <- 
  s_w_range_proc2 %>% 
  mutate(growth_ratio = r_w / r_s) %>% 
  filter(variables == "N_w" & b_stocks == "b_values_w") %>% 
  mutate(p_w = as.factor(p_w)) %>% 
  filter(p_w %in% c("1", "10", "50", "100"))

test <- 
  ggplot() + 
  geom_raster(data = s_w_range_proc_subset2 %>% filter(b_v_k > 0),
              aes(x = growth_ratio, y = p_s, fill = b_v_k)) +
  # geom_point(data = s_w_range_proc_subset2 %>% filter(b_v_k > -0.0015 & b_v_k <= 0),
  #             aes(x = growth_ratio, y = p_s, color = b_v_k)) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.25)) +
  facet_wrap(~ p_w,
             ncol = 2,
             labeller = labeller(p_w = p_w_labs2)) +
...
output_weak_test %>%
  ggplot() +
  geom_raster(aes(x = p_w_star, y = N_w_star)) +
...
output_fcn_test_ps %>%
  ...
  filter(N_w_star_2 >= 0) %>% 
  ggplot() +
  geom_raster(aes(x = growth_ratio, y = round(p_w_star,3), fill = round(N_w_star_2, 3)))
```
