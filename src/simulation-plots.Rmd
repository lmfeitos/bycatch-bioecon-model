...
s_w_range_proc_plot <-
  ggplot(data = s_w_range_proc %>% 
           filter(p_w < 1 & st_stock_rev <= 8 & vuln_ratio_w <= 4) %>% 
           distinct_all()) +
  geom_tile(aes(x = vuln_ratio_w, y = st_stock_rev, fill = log1p(b_v_k_w), width = .04)) +
...
w_s_range_proc_plot1 <-
  ...
  geom_tile(aes(x = vuln_ratio_w, y = w_stock_rev, fill = log1p(b_v_k_w), width = .04)) +
  scale_fill_paletteer_c("ggthemes::Green-Blue-White Diverging",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.1),
                         breaks = seq(0, 0.55, 0.1),
                         direction = -1,
...
for(i in 2:length(tset)) {
  ...
  dcost_ns <- (cost_ns8 * E8/C_s8) * dt
  dcost_nw <- (cost_nw8 * E8/C_w8) * dt
  dcost_sum <- (E8 * (cost_nw8 + cost_ns8)/(C_s8 + C_w8)) * dt
  drev_ns8 <- (A_s * (q_s * N_s8^beta_s8 * E8)^-f_s8 * q_s * N_s8 * E8/C_s8) * dt
  drev_nw8 <- (A_w * (q_w * N_w8^beta_w8 * E8)^-f_w8 * q_s * N_w8 * E8/C_w8) * dt
  drev_sum8 <- ((A_s * (q_s * N_s8^beta_s8 * E8)^-f_s8 * q_s * N_s8 * E8 + 
                  A_w * (q_w * N_w8^beta_w8 * E8)^-f_w8 * q_w * N_w8 * E8/(C_s8 + C_w8))) * dt
  ...
  Pi_s_simu8[i] <- pi_s8 + dPi_s8
  Pi_w_simu8[i] <- pi_w8 + dPi_w8
  cost_ns_simu8[i] <- cost_ns8 + dcost_ns
  cost_nw_simu8[i] <- cost_nw8 + dcost_nw
  cost_sum_simu8[i] <- cost_sum8 + dcost_sum
  rev_ns_simu8[i] <- rev_ns8 + drev_ns8
  rev_nw_simu8[i] <- rev_nw8 + drev_nw8
...
```{r, include = FALSE}
plot(x = N_s_simu8, y = cost_ns_simu8,
     col = "purple", las = 1,
     type = "l",
     lwd = 2,
     xlab = "Strong stock abundance",
     ylab = "Strong stock price, cost")
lines(x = N_s_simu8, y = rev_sum_simu8,
     col = "black", type = "l",
     lwd = 2)
...
cost_ns_df <- cbind(cost_ns_simu8, tset) %>% 
  as.data.frame() %>%
  pivot_longer(cost_ns_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = "5") 

cost_nw_df <- cbind(cost_nw_simu8, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cost_nw_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = "5") 

cost_sum_df <- cbind(cost_sum_simu8, tset) %>% 
  as.data.frame() %>% 
  pivot_longer(cost_sum_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = "5") 


rev_ns_df <- cbind(rev_ns_simu8, tset) %>%  
  as.data.frame() %>% 
  pivot_longer(rev_ns_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = "5") 

rev_nw_df <- cbind(rev_nw_simu8, tset) %>%  
  as.data.frame() %>%
  pivot_longer(rev_nw_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id ="5") 

rev_sum_df <- cbind(rev_sum_simu8, tset) %>%  
  as.data.frame() %>% 
  pivot_longer(rev_sum_simu8,
               names_to = "variable",
               values_to = "value") %>% 
  mutate(scenario_id = "5") 
...
var_full_price <- bind_rows(effort_df, nw_df, ns_df, catch_s_df, catch_w_df, cpue_s_df, cpue_w_df, profits_df,
                            profits_w_df, profits_s_df, cost_ns_df, cost_nw_df, cost_sum_df, rev_ns_df, rev_nw_df, rev_sum_df)
...
var_plot_price <- var_full_price %>%
  ...
  mutate(variable_clean = case_when(
    ...
    str_detect(variable, "C_w") ~ "Weak stock catch",
    str_detect(variable, "C_s") ~ "Strong stock catch",
    str_detect(variable, "cost_ns") ~ "Strong stock cost/catch",
    str_detect(variable, "cost_nw") ~ "Weak stock cost/catch",
    str_detect(variable, "cost_sum") ~ "Total cost/catch",
    str_detect(variable, "rev_ns") ~ "Strong stock revenue/catch",
    str_detect(variable, "rev_nw") ~ "Weak stock revenue/catch",
...
st_stock <- var_plot_price %>% 
  filter(scenario_id == "5" &
           variable_clean %in% c("Strong stock")) %>% 
...
st_stock_cost <- var_plot_price %>% 
  filter(scenario_id == "5" &
           variable_clean %in% c("Strong stock cost/catch")) %>% 
  pivot_wider(names_from = variable_clean,
              values_from = value) %>% 
  clean_names()
...
st_stock_rev <- var_plot_price %>% 
  filter(scenario_id == "5" &
           variable_clean %in% c("Strong stock revenue/catch")) %>% 
  pivot_wider(names_from = variable_clean,
              values_from = value) %>% 
  clean_names()
...
strong_st_1 <-
  st_join %>%   
  ggplot() +
  geom_point(aes(x = log(strong_stock), y = catch_rev_val, color = econ_var_catch),
            size = .5) +
  scale_x_continuous(breaks = seq(1.1, 1.4, .05)) +
  labs(x = "Log(Strong stock abundance (Ns))",
       y = "Strong stock revenue/catch, Log(Strong stock cost/catch)",
       color = "",
...
weak_st_1 <-
  w_join %>%   
  ggplot() +
  geom_point(aes(x = log1p(weak_stock), y = catch_rev_val, color = econ_var_catch),
            size = .5) +
