...
output_s_ranges_w_ranges <- read_csv(file.path(outdir, "output_s_ranges_w_ranges.csv"))

output_w_ranges_s_ranges <- read_csv(file.path(outdir, "output_w_ranges_s_ranges.csv"))
...
s_w_range_proc <- output_s_ranges_w_ranges %>% 
  pivot_wider(names_from = "variables",
              values_from = "values") %>% 
  mutate(b_values_s = N_s / k_s,
         b_values_w = N_w / k_w) %>% 
...
w_s_range_proc <- output_w_ranges_s_ranges %>% 
  pivot_wider(names_from = "variables",
              values_from = "values") %>% 
  mutate(b_values_s = N_s / k_s,
         b_values_w = N_w / k_w) %>% 
...
#               values_from = "r") %>% 
#   filter(variables == "N_w" & r_s >= r_w & values > 0 & p_w == 0 & p_s == 1) %>% 
#   ggplot() +  
...
#   scale_y_continuous(breaks = seq(0, 1.1, 0.2),
#                      expand = c(0,0)) +
#   scale_fill_gradientn(name = "B/K", colors = RColorBrewer::brewer.pal(9, "YlOrRd")) +
#   labs(x = "rw",
#        y = "rs",
#        title = "Weak stock positive equilibria ranges") +
#   theme_bw() +
...
#               values_from = "r") %>% 
#   filter(variables == "N_w" & r_s >= r_w & values > 0) %>% 
#   mutate(r_ratio = r_w/r_s,
#          mean_r_ratio = mean(r_ratio),
#          sd_r_ratio = sd(r_ratio)) 
```
...
s_w_zero_proc_plot <- 
  ...
  filter(variables == "N_w" & b_stocks == "b_values_w" & b_v_k > 0 & b_v_k <= 1) %>% 
  ggplot() + 
  geom_raster(aes(x = growth_ratio, y = p_s, fill = b_v_k)) +
...
```{r, echo = FALSE}
p_w_labs <- c("pw = 1", "pw = 4", "pw = 7", "pw = 10", "pw = 13")
names(p_w_labs) <- c(1, 4, 7, 10, 13)
...
s_w_range_proc_plot <-
  ...
  ggplot() + 
  geom_raster(aes(x = growth_ratio, y = p_s, fill = b_v_k)) +
  #geom_line(aes(x = growth_ratio, y = p_s, color = b_v_k)) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.1),
                         breaks = seq(0, 1, 0.05)) +
  facet_wrap(~ p_w,
             nrow = 1,
             labeller = labeller(p_w = p_w_labs)) +
  scale_x_continuous(breaks = seq(.5, 1, .25),
                     expand = c(0,0)) +
  scale_y_continuous(breaks = seq(0, 10, 1),
                     expand = c(0,0)) +
  labs(x = "Growth rate ratio (rw/rs)",
       y = "Strong stock price",
       fill = "Weak stock \nB/K",
       title = "Weak stock with non-zero value") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 10, color = "black"),
...
w_s_zero_proc_plot <- 
  ...
  filter(variables == "N_w" & b_stocks == "b_values_w" & b_v_k > 0 & b_v_k <= 1) %>% 
  ggplot() + 
  geom_raster(aes(x = growth_ratio, y = p_w, fill = b_v_k)) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.5),
  ...
  labs(x = "Growth rate ratio (rw/rs)",
       y = "Weak stock price",
       fill = "Weak stock \nB/K",
       title = "Strong stock with zero price") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 10, color = "black"),
...
```{r, echo = FALSE}
p_s_labs <- c("ps = 1", "ps = 4", "ps = 7", "ps = 10")
names(p_s_labs) <- c(1, 4, 7, 10)

w_s_range_proc_plot <-
  w_s_range_proc %>% 
  mutate(growth_ratio = r_w / r_s) %>% 
  filter(variables == "N_w" & b_stocks == "b_values_w" & b_v_k > 0) %>% 
  mutate(p_s = as.factor(p_s)) %>% 
  ggplot() + 
  geom_raster(aes(x = growth_ratio, y = p_w, fill = b_v_k),
              show.legend = T) +
  scale_fill_paletteer_c("viridis::viridis",
                          rescaler = ~ scales::rescale_mid(.x, mid = 0.05)) +
  facet_wrap(~ p_s,
             nrow = 1,
             labeller = labeller(p_s = p_s_labs)) +
  scale_x_continuous(breaks = seq(0.88, 1, 0.06),
                      expand = c(0,0)) +
  scale_y_continuous(breaks = seq(0, 10, 1),
                     expand = c(0,0)) +
  labs(x = "Growth rate ratio (rw/rs)",
       y = "Weak stock price",
       fill = "Weak stock \nB/K",
       title = "Strong stock with non-zero price") +
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.title = element_text(size = 10, color = "black"),
...
(s_w_zero_proc_plot +
  ...
  plot_annotation(tag_levels = "A",,
                  title = "Weak stock B/K under different strong and weak stock prices")
```
...
output_test_updated %>% 
  mutate(r_ratio = r_w / r_s) %>%
  filter(variables == "N_w" & b_stocks == "b_values_w" & b_v_k > 0 & b_v_k <= 1) %>% 
  mutate(mean_r_ratio = mean(r_ratio),
         sd_r_ratio = sd(r_ratio))
  
```


### Second test plot with only the weak stock equilibrium

```{r, echo = FALSE}
output_weak_test %>%
  ggplot() +
  geom_line(aes(x = p_w_star, y = N_w_star),
            size = 1) +
...
sc_1 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 0.1, c = 0.05)
sc_2 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 0.2, c = 0.05)
sc_3 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 0.4, c = 0.05)
sc_4 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 1, c = 0.05)
sc_5 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 1.33, c = 0.05)
sc_6 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 12, c = 0.05)
sc_7 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 14, c = 0.05)
sc_8 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 0.8, r_w = 0.71, p_s = 1, p_w = 10, c = 0.05)
...
eq_par <- data.frame(parameter = c("q_w", "q_s", "k_w", "k_s", "r_w", "r_s", "p_s", "p_w", "c"),
                     "PR_10" =  c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 0.1, 0.05),
                     "PR_5" =   c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 0.2, 0.05),
                     "PR_2.5" = c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 0.4, 0.05),
                     "PR_1" =   c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 1, 0.05),
                     "PR_0.75" =c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 1.33, 0.05),
                     "PR_0.5" = c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 2, 0.05),
                     "PR_0.25" =c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 4, 0.05),
                     "PR_0.1" = c(0.1, 0.1, 4, 2, 0.8, 0.71, 1, 10, 0.05))
...
sc_r_1 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.7, p_s = 1, p_w = 1, c = 0.05)
sc_r_2 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.88, p_s = 1, p_w = 1, c = 0.05)
sc_r_3 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.9, p_s = 1, p_w = 1, c = 0.05)
sc_r_4 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.92, p_s = 1, p_w = 1, c = 0.05)
sc_r_5 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.95, p_s = 1, p_w = 1, c = 0.05)
sc_r_6 <- run_model(q_s = 0.1, q_w = 0.1, k_s = 4, k_w = 2, r_s = 1, r_w = 0.97, p_s = 1, p_w = 1, c = 0.05)
...
eq_par <- data.frame(parameter = c("q_w", "q_s", "k_w", "k_s", "r_w", "r_s", "p_w", "p_s", "c"),
                     "RR_2" =    c(0.1, 0.1, 4, 2, 1, 0.7, 1, 1, 0.05),
                     "RR_1.75" = c(0.1, 0.1, 4, 2, 1, 0.88, 1, 1, 0.05),
                     "RR_1.5" =  c(0.1, 0.1, 4, 2, 1, 0.9, 1, 1, 0.05),
                     "RR_1.25" = c(0.1, 0.1, 4, 2, 1, 0.92, 1, 1, 0.05),
                     "RR_1.1" =  c(0.1, 0.1, 4, 2, 1, 0.95, 1, 1, 0.05),
                     "RR_1" =    c(0.1, 0.1, 4, 2, 1, 0.97, 1, 1, 0.05))
...
var_plot_r <- var_full_r %>%
  mutate(price_ratio = case_when(
    str_detect(scenario_id, "1") ~ "Rw/Rs = 0.7",
    str_detect(scenario_id, "2") ~ "Rw/Rs = 0.88",
    str_detect(scenario_id, "3") ~ "Rw/Rs = 0.9",
