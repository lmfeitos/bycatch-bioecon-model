...
var_plot_price <- var_full_price %>%
   mutate(f_ratio = case_when(
     str_detect(scenario_id, "1") ~ "f_w/f_s = 0.2",
     str_detect(scenario_id, "2") ~ "f_w/f_s = 0.6",
     str_detect(scenario_id, "3") ~ "f_w/f_s = 1",
     str_detect(scenario_id, "4") ~ "f_w/f_s = 1.4",
     str_detect(scenario_id, "5") ~ "f_w/f_s = 0.44",
     str_detect(scenario_id, "6") ~ "f_w/f_s = 0.44",
     str_detect(scenario_id, "7") ~ "f_w/f_s = 0.44",
     str_detect(scenario_id, "8") ~ "f_w/f_s = 0.44"
   )) %>% 
   mutate(beta_ratio = case_when(
     str_detect(scenario_id, "1") ~ "beta_w/beta_s = 1.25",
     str_detect(scenario_id, "2") ~ "beta_w/beta_s = 1.25",
     str_detect(scenario_id, "3") ~ "beta_w/beta_s = 1.25",
     str_detect(scenario_id, "4") ~ "beta_w/beta_s = 1.25",
     str_detect(scenario_id, "5") ~ "beta_w/beta_s = 0.35", 
     str_detect(scenario_id, "6") ~ "beta_w/beta_s = 0.71",
     str_detect(scenario_id, "7") ~ "beta_w/beta_s = 1",
     str_detect(scenario_id, "8") ~ "beta_w/beta_s = 1.6"
   )) %>% 
  mutate(variable_clean = case_when(
    str_detect(variable, "N_w") ~ "Weak stock",
    str_detect(variable, "N_s") ~ "Strong stock",
...
equal_scales_EWS_fcn <-
  ...
  facet_wrap(f_ratio ~ beta_ratio,
             nrow = 2,
             labeller = labeller(f_ratio = f_labels,
                                     beta_ratio = beta_labels)) +
