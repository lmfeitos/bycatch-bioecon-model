...
for(i in 2:nyr){
  ...
  # Calculate exploitation this year
  # C = qEN
  # C/N = qE
  # FMSY = qE
  F_now <- fmsy/q
  ...
  # Calculate catch this year
  C_now <- q * F_now * N_prev

  # Calculate biomass this year
  SP_now <- r*N_prev*(1-N_prev/K)
  N_now <- N_prev + SP_now - C_now
...
# Derive some things
data <- data %>%
  mutate(depletion=N/K,
         cpue=C/E)


# Plot
plot(N ~ year, data, ylim=c(0,10))
plot(cpue ~ year, data)
plot(C ~ year, data)
plot(E ~ year, data)
...
for(i in 2:length(tset)) {
  ...
  cpue_s_simu5[i] <- cpue_s5 + dcpue_s5
  cpue_w_simu5[i] <- ifelse(N_w5 + dN_w5 < 0, 0, cpue_w5 + dcpue_w5)
  Pi_sum_simu5[i] <- pi5 + dPi5
  Pi_s_simu5[i] <- pi_s5 + dPi_s5
...
for(i in 2:length(tset)) {
  ...
  cpue_s_simu7[i] <- cpue_s7 + dcpue_s7
  cpue_w_simu7[i] <- ifelse(N_w7 + dN_w7 < 0, 0, cpue_w7 + dcpue_w7)
  Pi_sum_simu7[i] <- pi7 + dPi7
  Pi_s_simu7[i] <- pi_s7 + dPi_s7
...
for(i in 2:length(tset)) {
  ...
  cpue_s_simu8[i] <- cpue_s8 + dcpue_s8
  cpue_w_simu8[i] <- ifelse(N_w8 + dN_w8 < 0, 0, cpue_w8 + dcpue_w8)
  Pi_sum_simu8[i] <- pi8 + dPi8
  Pi_s_simu8[i] <- pi_s8 + dPi_s8
